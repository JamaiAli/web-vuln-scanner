...
Afin d'assurer une sécurité globale de l'application, ce scanner détecte activement trois des vulnérabilités les plus critiques du Web :

- **SQL Injection (SQLi) :** Une faille permettant à un attaquant d'interférer avec les requêtes de la base de données de l'application. Elle peut permettre de lire, modifier ou supprimer des données sensibles, voire de prendre le contrôle du serveur. Le scanner teste les points d'entrée avec des payloads spécifiques pour observer les erreurs ou les comportements anormaux.
